...
void ColladaParser::parse_polymesh(XMLElement* xml, PolymeshInfo& polymesh) {
  ...
  if (!e_polylist) { // with triangle support
...
bool ColladaWriter::writeScene( DynamicScene::Scene& scene, const char* filename )
{
   ofstream out( filename );
   if( !out.is_open() )
   {
      cerr << "WARNING: Could not open file " << filename << " for COLLADA export!" << endl;
      return false;
   }
...
void ColladaWriter::writeFooter( ofstream& out )
{
   out << "</COLLADA>" << endl;
}
