...
namespace CMU462 { namespace DynamicScene {
    ...
    void Mesh::linearBlendSkinning(bool useCapsuleRadius, double time)
    {
      // Implement Me! (Task 3a, Task 3b)
      if (skeleton->joints.size() == 1) return;
      else if (time == 0.0) {
        restoreMesh();
        return;
      }
      ...
      size_t i = 0;
      for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); ++v) {
        vector<LBSInfo> lbs_vector;
        lbs_vector.clear();
        double sum_dist_inv = 0.0;
        // Iterate through all joints in skeleton
        for (Joint* j : skeleton->joints) {
          LBSInfo lbs;
          // Get v coordinates in local frame
          Vector3D v_local = v->bindPosition - j->getBindTransformation() * Vector3D();
          // Calculate fields in lbs
          lbs.distance = distance_to_bone(v_local, j->axis);
          if ((!useCapsuleRadius) || 					// Naive LBS
              (useCapsuleRadius && lbs.distance < j->capsuleRadius)) {  // LBS with threshold
            lbs.blendPos = j->getTransformation() *
                           j->SceneObject::getTransformation() *
                           v_local;
            // add lbs to lbs_vector
            lbs_vector.push_back(lbs);
            // update sum of distances
            sum_dist_inv += 1./ lbs.distance;
          }
        }
        // Update vertex position
        Vector3D newPos = Vector3D();
        for (LBSInfo l : lbs_vector) {
          newPos += l.blendPos * ((1./ l.distance) / sum_dist_inv);
        }
        v->position = newPos;
        ++i; // Increment i for accessing old_vPos
      }
    }

    // Compute the distance from a skin vertex to a given joint axis
    double Mesh::distance_to_bone(Vector3D v_local, Vector3D joint_axis) const {
      double dotproduct = dot(v_local, joint_axis);
      double maxdot = joint_axis.norm2();
      if (0 < dotproduct && dotproduct < maxdot) {
        return (v_local - (joint_axis * dotproduct / maxdot)).norm();
      }
      else if (dotproduct <= 0) {
        return v_local.norm();
      }
      else {
        return (v_local - joint_axis).norm();
      }
    }

    void Mesh::forward_euler(float timestep, float damping_factor) {
      // Implement Me! (Task 4)
      vector<float> old_offset;
      vector<float> old_velocity;
      vector<float> u_acc;

      // Store old values
      for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); ++v) {
        old_offset.push_back(v->offset);
        old_velocity.push_back(v->velocity);
        u_acc.push_back(v->laplacian() - damping_factor*v->velocity);
      }
      // Update
      int i = 0;
      for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); ++v) {
        v->velocity += timestep * u_acc[i];
        v->offset += timestep * old_velocity[i];
        ++i;
      }
    }
    ...
    void Mesh::draw_faces_normal() const {
      ...
      for (FaceCIter f = mesh.facesBegin(); f != mesh.facesEnd(); f++) {
        ...
        float faceColor0 = (normal.x + 1) / 2,
            faceColor1 = (normal.y + 1) / 2,
            faceColor2 = (normal.z + 1) / 2;

        glColor3f(faceColor0, faceColor1, faceColor2);
        glBegin(GL_POLYGON);
    ...
    void Mesh::draw_faces_lighting() const {
      ...
      for (FaceCIter f = mesh.facesBegin(); f != mesh.facesEnd(); f++) {
        ...
        if (!smooth) {
          if (style != defaultStyle) {
            glDisable(GL_LIGHTING);
            style->style_face();
          } else if (mesh.displayVSA) {
            glDisable(GL_LIGHTING);
            // Get color representation of VSA
            Color proxyColor = f->getProxyColor();
            glColor4fv(&proxyColor.r);
          }
        }
        ...
        if (style != defaultStyle || mesh.displayVSA) {
          glEnable(GL_LIGHTING);
        }
      }
    }
    ...
    void Mesh::resample(size_t numProxy, size_t numIterations, double edgeSplitThreshold) {
      for( FaceCIter f = mesh.facesBegin(); f != mesh.facesEnd(); f++ )
      {
        if( f->degree() != 3 )
        {
          cerr << "Warning: refusing to apply VSA to surface with non-triangular faces!" << endl;
          cerr << "(Try triangulating first.)" << endl;
          return;
        }
      }
      resampler.resample(mesh, numProxy, numIterations, edgeSplitThreshold);
      scene->selected.clear();
      scene->hovered.clear();
      scene->elementTransform->target.clear();
    }
    ...
    void Mesh::setSelection( int pickID, Selection& selection )
    {
      ...
      if( idToElement.size() > pickID - idOffset )
      {
        selection.clear();
        selection.object = this;
        selection.element = idToElement[ pickID - idOffset ];
      }
    }
