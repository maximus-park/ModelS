...
namespace CMU462 { namespace DynamicScene {
    ...
    void Scene::render_in_normal_mode() {
    ...
    void Scene::render_in_shading_mode() {
      for (SceneObject *obj : objects) {
        if (obj->isVisible && !obj->isGhosted) {
          obj->draw_shading();
        }
    ...
    void Scene::render_splines_at(double time, bool pretty, bool useCapsuleRadius)
    {
      ...
      for (SceneObject *obj : objects)
      {
        ...
        if (mesh != nullptr)
        {
          mesh->linearBlendSkinning(useCapsuleRadius, time);
        }
      }
    ...
    void Scene::resample_selected_mesh(size_t numProxy, size_t numIterations, double edgeSplitThreshold) {
      if( selected.object == nullptr || selected.element == nullptr ) return;
      Mesh* m = dynamic_cast<Mesh*>( selected.object );
      if( m ) m->resample(numProxy, numIterations, edgeSplitThreshold);
      clearSelections();
    }
