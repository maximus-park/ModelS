...
namespace CMU462 {
  ...
  float Vertex::laplacian() const
  {
    ...
    do {
	VertexCIter j = h->twin()->vertex();
	VertexCIter v_alpha = h->next()->next()->vertex();
	VertexCIter v_beta  = h->twin()->next()->next()->vertex();

	Vector3D alphai = this->position - v_alpha->position;
	Vector3D alphaj =    j->position - v_alpha->position;
	float cos_alpha = dot(alphai, alphaj) / (alphai.norm()*alphaj.norm());
	float cot_alpha = cos_alpha / sinf(acos(cos_alpha));

	Vector3D betai = this->position - v_beta->position;
	Vector3D betaj =    j->position - v_beta->position;
	float cos_beta = dot(betai, betaj) / (betai.norm()*betaj.norm());
	float cot_beta = cos_beta / sinf(acos(cos_beta));

	lap += (cot_alpha + cot_beta) * (j->offset - this->offset);
	h = h->twin()->next();
  ...
  Color Face::getProxyColor() const {
    if (label < 0) return Color(0, 0, 0, 0);
    stringstream ss;
    ss << label;
    size_t hashVal = hash<string>{}(ss.str());
    int mask = (1 << 16) - 1;
    int base = 1 << 16;
    float r, g, b;
