...
namespace CMU462 {
  ...
  void Camera::compute_position() {
    ...
    // Debug output
//    fprintf(stdout, "camera pos: x=%lf, y=%lf, z=%lf\n", pos.x, pos.y, pos.z);
//    fprintf(stdout, "target pos: x=%lf, y=%lf, z=%lf\n", targetPos.x, targetPos.y, targetPos.z);
//    fflush(stdout);
  }
  ...
  Ray Camera::generate_ray(double x, double y) const {
    ...
    Vector3D direction = Vector3D(x-0.5, y-0.5, -1); // center of screen is (0.5, 0.5)
    Vector3D ray_ori = pos, ray_dir = c2w * direction;
    ray_dir.normalize();
