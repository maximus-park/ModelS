...
void usage(const char* binaryName) {
  ...
  printf("  -p  <INT>        VSA Number of Proxies\n");
  printf("  -i  <INT>        VSA Number of Iterations\n");
  printf("  -d  <FLOAT>      VSA Meshing Edge Split Threshold\n");
...
int main( int argc, char** argv ) {
  ...
  while ( (opt = getopt(argc, argv, "s:l:t:m:e:p:i:d:h")) != -1 ) {  // for each option...
    switch ( opt ) {
      case 's':
      ...
      case 'e':
        ...
        break;
      case 'p':
        config.vsa_num_proxy = atoi(optarg);
        break;
      case 'i':
        config.vsa_num_iterations = atoi(optarg);
        break;
      case 'd':
        config.vsa_split_threshold = atof(optarg);
        break;
      default:
        usage(argv[0]);
        return 1;
