...
struct AppConfig {
  ...
  AppConfig () {
    ...
    vsa_num_proxy = 50;
    vsa_num_iterations = 10;
    vsa_split_threshold = 0.5;
  }
  ...
  size_t vsa_num_proxy;
  size_t vsa_num_iterations;
  double vsa_split_threshold;
...
class Application : public Renderer {
  ...
  */
  ...
  /*****************************************************************************************/
  void drawContour(vector<vector<Vector3D>*> contour_paths);
  void drawShading(unsigned char *shade_buf);
  void drawShadingBuf(vector<vector<Vector3D>*> shading_paths, unsigned char *shade_buf,
		      unsigned char *result_buf, int level);
  void getShadeBuffer(unsigned char *shade_buf);
  void processShadeBuffer(unsigned char *shade_buf, unsigned char *skeleton_buf);
  void ToonShading(unsigned char *shade_buf);
  inline Vector3D lookNeighbour(Vector3D curr, unsigned char *buf);
  inline Vector3D startNewPath(unsigned char *result_buf);
  void constructPath(vector<vector<Vector3D>*> &paths, unsigned char *result_buf);
  void computeContour(unsigned char *result_buf, unsigned char *normal_buf, float *depth_buf);
  void thinning(int w, int h, unsigned char *imgBuf);
  unsigned char cut_off = 120;
  bool toon = false;
  float light_theta = 45.f; float light_phi = 75.f;
