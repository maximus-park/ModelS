...
namespace CMU462 {
  ...
  Spectrum PathTracer::trace_ray(const Ray &r, size_t& depth) {

    // Recursion depth control
    if (depth > max_ray_depth) return Spectrum();
    //fprintf(stdout, "max_depth=%zu, depth=%zu\n", max_ray_depth, depth);
    depth++;
    ...
    if (!bvh->intersect(r, &isect)) {
      ...
      if(envLight) return envLight->sample_dir(r);
      return Spectrum();
    }
	...
	double avg_factor = 1.0 / num_light_samples;
	for (int i=0; i<num_light_samples; i++) {
	    ...
	    Ray r = Ray(hit_p + EPS_D * dir_to_light, dir_to_light, dist_to_light);

	    if (!bvh->intersect(r)) L_out += f * light_L * cos_theta * avg_factor * (1.f / pdf);
	}
      }
    ...
    float pdf_indirect;
    isect.bsdf->sample_f(w_out, &w_in_indirect, &pdf_indirect);
    Spectrum f = isect.bsdf->f(w_out, w_in_indirect);
    Vector3D w_in_indirect_w = o2w * w_in_indirect;
    double cos_theta_in = fabs(w_in_indirect[2]);


    // Apply Russian roulette, rand_f factor is adjustable
    float terminateProbablity = std::max(0.f, std::min((1.f - f.illum()), 1.f));
    float rand_f = (float) (std::rand()) / RAND_MAX;
    if (rand_f < terminateProbablity) return L_out;

    Ray r_indirect = Ray(hit_p + EPS_D * w_in_indirect_w, w_in_indirect_w);
    float adj = 1.f / (pdf_indirect * (1 - terminateProbablity));
    return L_out + (f * trace_ray(r_indirect, depth)) * cos_theta_in * adj;
  ...
  Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
    ...
    double ratio = 1. / (double) max(w, h);

    Vector2D p = Vector2D((double) x, (double) y);

    if (num_samples == 1) {
	p += Vector2D(0.5, 0.5);
	if (w > h)
	  return trace_ray(camera->generate_ray(p.x * ratio, (p.y+(w-h)/2) * ratio), depth);
	else
	  return trace_ray(camera->generate_ray((p.x+(h-w)/2) * ratio, p.y * ratio), depth);
    }

    else {
	Spectrum s = Spectrum(0, 0, 0);
	for (int i = 0; i < num_samples * num_samples; i++){
	    Vector2D p0 = p + gridSampler->get_sample();
	    depth = 0; // Restore depth count
	    if (w > h)
		s += trace_ray(camera->generate_ray(p0.x * ratio, (p0.y+(w-h)/2) * ratio), depth);
	    else
		s += trace_ray(camera->generate_ray((p0.x+(h-w)/2) * ratio, p0.y * ratio), depth);
	}
	return s *= (1. / (double) (num_samples * num_samples));
    }
